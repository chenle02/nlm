...
generate_and_download_audio() {
  ...
  # Loop until audio_overview_<id>.wav is generated by nlm audio-get
  local wavfile
  while true; do
    # Invoke audio-get (it writes audio_overview_<id>.wav when ready)
    nlm audio-get "$id" || true
    wavfile="audio_overview_${id}.wav"
    if [[ -f "$wavfile" ]]; then
